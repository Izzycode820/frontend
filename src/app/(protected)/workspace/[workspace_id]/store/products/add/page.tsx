  ...
  const [createProduct, { loading }] = useMutation(CreateProductDocument, {
    update(cache, { data }) {
      if (data?.createProduct?.success && data.createProduct.product) {
        ...
        try {
          // Manually update the list cache to include the new product
          // We target the default VIEW (page 1, no filters)
          ...
          const existingData = cache.readQuery({
            query: require('@/services/graphql/admin-store/queries/products/__generated__/GetProductsList.generated').GetProductsListDocument,
