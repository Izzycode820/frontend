...
export default function UniversalEditor({
  ...
  console.log('📋 [UniversalEditor] Received puckData:', JSON.stringify(puckData, null, 2));
  ...
  const loadMasterConfig = async () => {
    try {
      ...
      console.log('🔵 [UniversalEditor] Loading master config for:', themeSlug);

      // Load master config from registry (shared by all users)
      const masterConfig = await loadThemeConfig(themeSlug);

      console.log('✅ [UniversalEditor] Master config loaded', masterConfig.components ? Object.keys(masterConfig.components) : 'NO COMPONENTS');
      setConfig(masterConfig);
      setIsLoading(false);

    } catch (err: any) {
      console.error('❌ [UniversalEditor] Failed to load config:', err);
  ...
  const handleSave = (newData: Data) => {
    console.log('💾 [UniversalEditor] Saving changes');
    setData(newData);
    onSave?.(newData);
  };
  ...
  const handlePublish = async (newData: Data) => {
    // Save the data first
    console.log('💾 [UniversalEditor] Saving before publish');
    setData(newData);
    await onSave?.(newData);

    // Then call publish to change status
    console.log('🚀 [UniversalEditor] Publishing theme');
    onPublish?.();
  };
  ...
  return (
    <div className={`puck-editor ${className}`} style={{ height: '100vh' }}>
      <Puck
        config={config}
        data={data}
