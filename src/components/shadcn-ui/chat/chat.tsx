...
import { BlueprintBubble } from "@/components/workspace/store/whatsapp/inbox/BlueprintBubble"
...
function ChatProvider({
  ...
  const [replyingTo, setReplyingTo] = React.useState<ChatMessageData | null>(null)
...
function ChatVoiceMessage({ voice, isOutgoing }: { voice: NonNullable<ChatMessageData["voice"]>; isOutgoing: boolean }) {
  ...
  const DEFAULT_WAVEFORM = [
    0.2, 0.5, 0.3, 0.4, 0.6, 0.3, 0.4, 0.5, 0.7, 0.4, 0.3, 0.5, 0.2, 0.4, 0.3, 0.5,
    0.6, 0.3, 0.4, 0.5, 0.7, 0.4, 0.3, 0.5, 0.2, 0.4, 0.3, 0.5, 0.6, 0.3, 0.4, 0.5,
  ]
  const waveform = voice.waveform?.length ? voice.waveform : DEFAULT_WAVEFORM
...
function ChatLocationMessage({
  location,
  isOutgoing,
}: {
  location: NonNullable<ChatMessageData["location"]>
  isOutgoing: boolean
}) {
  const mapUrl = `https://maps.google.com/maps?q=${location.latitude},${location.longitude}&z=15&output=embed`
  return (
    <div className="mt-1.5 overflow-hidden rounded-lg border border-[var(--chat-border-strong)] bg-[var(--chat-bg-sidebar)]">
      <div className="aspect-video w-full overflow-hidden bg-[var(--chat-bg-main)]">
         <iframe
          title="Location Map"
          width="100%"
          height="100%"
      ...
      <div className="p-3">
        <div className="flex items-center gap-2">
          <MapPin className="size-4 text-[var(--chat-accent)]" />
          <p className="text-[13px] font-semibold text-[var(--chat-text-primary)] truncate">
            {location.name || "Location Shared"}
          </p>
        </div>
        {location.address && (
          <p className="mt-1 text-[12px] text-[var(--chat-text-secondary)] line-clamp-2">
            {location.address}
          </p>
        )}
        <a
          ...
          className="mt-2 block text-center text-[11px] font-medium text-[var(--chat-accent)] hover:underline"
        >
          View on Google Maps
        </a>
      </div>
    ...
    >
      ...
      <div className="flex max-w-[75%] flex-col">
        ...
        <div className="relative">
          ...
          {/* Thinking Neon Capsule (Customer Only) — Floating to the RIGHT and extending rightward */}
          {message.senderType === 'customer' && message.blueprintSteps && message.blueprintSteps.length > 0 && (
            <div className="absolute left-[calc(100%+1.5rem)] top-4 flex items-center group/capsule z-20 pointer-events-auto">
              {/* SVG Annotation Line (String) — Straight line pointing LEFT to bubble */}
              <svg className="absolute right-[100%] top-1/2 -translate-y-1/2 w-6 h-4 pointer-events-none opacity-40 group-hover/capsule:opacity-100 transition-opacity" viewBox="0 0 24 16">
                <line 
                  x1="24" y1="8" x2="0" y2="8" 
                  stroke="currentColor" 
                  strokeWidth="1.5" 
                  strokeLinecap="round"
                  className="text-green-500/40"
                />
                <circle cx="0" cy="8" r="2.5" className="fill-green-500/60" />
              </svg>
              
              <BlueprintBubble steps={message.blueprintSteps} isAiThinking={isAiThinking} />
            </div>
          )}

          <div
            className={cn(
              "chat-bubble relative px-4 py-2.5 shadow-sm transition-all animate-in fade-in slide-in-from-bottom-1 duration-300",
              message.senderType === 'customer' 
                ? cn(
                    "bg-zinc-900 text-zinc-100 border border-zinc-800 rounded-[0px_16px_16px_16px] shadow-lg shadow-black/20",
                    message.blueprintSteps && message.blueprintSteps.length > 0 && "border-green-500/40 ring-1 ring-green-500/10"
                  )
                : message.senderType === 'merchant'
                ? "bg-white text-zinc-950 border border-zinc-200 rounded-[16px_0px_16px_16px] shadow-md"
                : message.senderType === 'workman'
                ? "bg-zinc-900 border-l-[3px] border-indigo-500 text-zinc-100 rounded-[0px_16px_16px_16px] shadow-lg"
                : isOutgoing
                ? "bg-white text-zinc-950 border border-zinc-200 rounded-[16px_0px_16px_16px]"
                : "bg-zinc-900 text-zinc-100 border border-zinc-800 rounded-[0px_16px_16px_16px]",
              radiusClass
            )}
          >
            {/* Quoted reply */}
            ...
            {/* Meta Interactive Content (Zoko-style) */}
            ...
            {/* Location message */}
            {message.location && (
              <ChatLocationMessage location={message.location} isOutgoing={isOutgoing} />
            )}
...
function ChatMessageGroup({ group, className, isAiThinking }: ChatMessageGroupProps) {
...
function ChatMessages({
  ...
  return (
    <div
      className={cn(
        "chat-messages relative flex flex-1 flex-col overflow-hidden bg-[#0B141A]",
        className
      )}
    >
      {/* WhatsApp-style subtle pattern overlay */}
      <div className="absolute inset-0 opacity-[0.06] pointer-events-none" 
           style={{ backgroundImage: 'url("https://w0.peakpx.com/wallpaper/580/650/HD-wallpaper-whatsapp-background-dark-pattern-thumbnail.jpg")', backgroundSize: '400px' }} 
      />
      {/* Scrollable area */}
      <div
        ref={containerRef}
      ...
      >
        <div className="w-full">
          ...
          {/* Custom children (e.g. Blueprint Bubbles) */}
          {children}
        </div>
      </div>
...
const ChatComposer = React.memo(function ChatComposer({
  ...
  const { replyingTo, onCancelReply } = useChatContext()
  const { textareaRef, resize } = useAutoResize({ maxRows: 6 })
  // Initialize local value from prop, but allow local overrides for speed
  const [localValue, setLocalValue] = React.useState(controlledValue || "")

  // Sync local value when external value changes (e.g. AI draft applied)
  React.useEffect(() => {
  ...
  // Internal value change handler
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    const val = e.target.value
    setLocalValue(val) // Immediate UI update
    
    // Defer parent update and resize to keep keystrokes snappy
    onValueChange?.(val)
    resize()
  }

  const addFiles = React.useCallback((arr: FileList | File[]) => {
    const items = Array.from(arr).map((file) => ({
      file,
  ...
  const handleSend = React.useCallback(() => {
    ...
    if (trimmed || files.length > 0) onSend?.(trimmed)
    // Always clear immediately — don't wait for the parent to sync back.
    // If the input is controlled, also notify the parent so its state
    // stays in sync (prevents the textarea appearing stuck after send).
    setLocalValue("")
    onValueChange?.("")
    setFiles([])
    stopTyping()
    if (textareaRef.current) textareaRef.current.style.height = "auto"
  }, [value, files, disabled, onSend, onValueChange, stopTyping, textareaRef])
  ...
  return (
    <div
      className={cn("chat-composer sticky bottom-0 z-10", className)}
    ...
    >
      ...
      {isDragging && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-primary/10 border-2 border-dashed border-primary rounded-xl backdrop-blur-sm">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-sm font-bold text-primary">Drop files to upload</span>
          </div>
        </div>
      )}
      ...
      {/* File preview strip */}
      {files.length > 0 && (
        <div className="flex gap-3 overflow-x-auto border-t border-border bg-card px-4 pt-3 pb-2">
          {files.map((f) => (
      ...
      {/* Composer body */}
      <div className="border-t border-border bg-card px-3 py-2 shadow-2xl">
        <div className="w-full">
          <div className="flex items-end gap-2">
            <div className="relative">
              <button
                ...
                className={cn(
                  "flex size-10 items-center justify-center rounded-full border border-border bg-background text-muted-foreground transition-all hover:bg-primary/10 hover:text-primary",
                  showAttachMenu && "rotate-45 bg-primary/20 text-primary"
                )}
              >
              ...
              {showAttachMenu && (
                <div className="absolute bottom-full left-0 mb-3 w-48 overflow-hidden rounded-2xl border border-border bg-card p-1 shadow-2xl animate-in fade-in slide-in-from-bottom-2">
                  <button
                    onClick={() => { fileInputRef.current?.click(); setShowAttachMenu(false) }}
                    className="flex w-full items-center gap-3 px-3 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-primary/10 hover:text-primary rounded-xl"
                  >
            ...
            <div className="relative flex flex-1 items-end rounded-[22px] border border-border bg-background">
              <textarea
                ref={textareaRef}
                value={value}
                ...
                disabled={disabled}
                rows={1}
                className="flex-1 resize-none bg-transparent py-[10px] pl-4 pr-12 text-[15px] leading-[22px] tracking-[-0.01em] text-foreground placeholder:text-muted-foreground focus:outline-none disabled:opacity-50"
              ...
              {!hasContent && onVoiceRecord ? (
                <button
                  ...
                  disabled={disabled}
                  className="absolute bottom-[6px] right-[6px] flex size-8 items-center justify-center rounded-full text-muted-foreground transition-colors hover:text-primary"
