...
function ChatHeader({ title, subtitle, avatar, actions, onBack, className }: ChatHeaderProps) {
  return (
    <header className={cn("sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-card px-4 py-3 shadow-sm", className)}>
      {onBack && (
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <span className="truncate text-[15px] font-bold tracking-tight text-foreground">{title}</span>
...
const ConversationItem = React.memo(function ConversationItem({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "mx-2 flex w-[calc(100%-16px)] items-center gap-3 rounded-xl px-3 py-3 text-left transition-all duration-200 mb-1",
        isActive 
    ...
    >
      <div className="relative shrink-0">
        <div className="flex size-11 items-center justify-center rounded-full bg-muted text-[13px] font-semibold text-foreground">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {convo.lastMessageTime && (
              <span className="text-[11px] text-muted-foreground">{convo.lastMessageTime}</span>
            )}
          </div>
        </div>
        <div className="flex items-center justify-between">
