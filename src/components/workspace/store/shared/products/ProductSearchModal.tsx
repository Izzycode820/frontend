...
export function ProductSearchModal(props: ProductSearchModalProps) {
  ...
  const t = useTranslations('Shared.products');
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {mode === 'order' && (
                          <p className="text-xs text-muted-foreground">
                            FCFA {formatCurrency(product.price)} · {product.inventoryQuantity} {t('available').toLowerCase()}
                          </p>
                        )}
                      </div>
                      {isSelected && (
            ...
            <table className="w-full">
              <thead className="bg-muted/50 sticky top-0">
                <tr className="border-b">
                  ...
                  <th className="text-left p-3 text-sm font-medium">{t('product')}</th>
                  {mode === 'order' && (
                    <>
                      <th className="text-right p-3 text-sm font-medium">{t('available')}</th>
                      <th className="text-right p-3 text-sm font-medium">{t('price')}</th>
                    </>
                  )}
