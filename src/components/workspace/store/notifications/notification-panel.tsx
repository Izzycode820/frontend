 ...
 * - Desktop: Side Sheet (slide from right)
...
function NotificationPanelInner({
    ...
    const { data, loading, refetch } = useQuery(
        GetWorkspaceNotificationsDocument,
        {
            variables: { limit: 50 },
            skip: !open,
        }
    ...
    const NotificationContent = (
        <>
            <ScrollArea className="flex-1 px-4 pb-4 -mx-4">
                <div className="px-4">
                    <NotificationList
                        loading={loading}
    ...
    return (
        <Drawer open={open} onOpenChange={onOpenChange}>
            <DrawerContent className="max-h-[85vh] flex flex-col">
                ...
                {NotificationContent}
                {/* Add a bit of padding at bottom for safe area if needed */}
                <div className="pb-4" />
            </DrawerContent>
        </Drawer>
    );
