...
export default function MenusListContainer() {
  ...
  const handleDeleteMenu = async (menuId: string) => {
      ...
      try {
        ...
        toast.error(t('menus.deleteError') + ': ' + err.message);
      }
  };

  if (loading) return <div className="p-8 text-center text-muted-foreground">{t('common.loading')}</div>;
  if (error) return <div className="p-8 text-center text-destructive">{t('common.error')}: {error.message}</div>;
  ...
  return (
    <div className="space-y-4 px-4 lg:px-6 py-6">
       <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">{t('menus.title')}</h1>
       ...
       {menus.length === 0 ? (
          <Card>
            <CardContent className="pt-6 text-center py-12">
               <p className="text-muted-foreground">{t('menus.noMenus')}</p>
               <Button variant="link" onClick={handleAddMenu}>{t('menus.createFirst')}</Button>
            </CardContent>
          </Card>
       ) : isMobile ? (
          <MobileMenusList
