...
export function BillingPage() {
  ...
  const { status, tier, billingCycle, renewSubscription } = useSubscription();
  ...
  const handleReactivate = () => {
    // Restricted subscriptions cannot be renewed, must create new one
    // Pass current plan details to pre-fill checkout
  ...
  return (
    <div className="w-full max-w-[1000px] mx-auto px-4 md:px-6">
      <div className="space-y-6">
        ...
        {/* Restricted Subscription Alert */}
        {(status === 'restricted' || status === 'past_due') && (
          <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2">
            ...
            <Button
              className="bg-black hover:bg-neutral-800 text-white shrink-0 w-full sm:w-auto"
