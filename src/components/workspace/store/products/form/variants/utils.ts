 ...
 * 
 * Uses deterministic IDs based on option values for stability across regeneration
 */
export function generateVariantCombinations(
  ...
  // Helper: Generate deterministic ID from option values
  const generateId = (option1?: string, option2?: string, option3?: string): string => {
    const parts = ['variant', option1, option2, option3].filter(Boolean);
    return parts.join('-').replace(/\s+/g, '_'); // Replace spaces with underscores
  };

  // Special handling for 3 options - skip middle level
  if (validOptions.length === 3) {
    validOptions[0].optionValues.forEach((value1) => {
      const parentId = generateId(value1);
      ...
      validOptions[1].optionValues.forEach((value2) => {
        validOptions[2].optionValues.forEach((value3) => {
          const childId = generateId(value1, value2, value3);
          children.push({
            id: childId,
            option1: value1,
            option2: value2,
  ...
  function buildHierarchy(
    ...
    currentOption.optionValues.forEach((value) => {
      ...
      const variantId = generateId(optionValues.option1, optionValues.option2, optionValues.option3);
...
/**
 * Reconstruct hierarchical structure from flat variants (EDIT mode)
 * Inverse of flattenVariantsForMutation - rebuilds UI structure from GraphQL data
 * 
 * This function takes flat variants loaded from the backend and rebuilds the
 * hierarchical parent-child structure that the UI expects, while preserving
 * all existing data (inventory, images, prices, etc.)
 *
 * @param flatVariants - Flat array of variants from GraphQL (with all data intact)
 * @param options - Product options to determine hierarchy structure
 * @returns Hierarchical variant tree matching UI expectations
 */
export function reconstructHierarchyFromFlat(
  flatVariants: VariantFormState[],
  options: ProductOption[]
): VariantFormState[] {
  if (flatVariants.length === 0 || options.length === 0) return [];

  const hierarchicalVariants: VariantFormState[] = [];

  // Helper: Generate deterministic ID (same as generateVariantCombinations)
  const generateId = (option1?: string, option2?: string, option3?: string): string => {
    const parts = ['variant', option1, option2, option3].filter(Boolean);
    return parts.join('-').replace(/\s+/g, '_');
  };

  // Special handling for 3 options - skip middle level (matches generateVariantCombinations)
  if (options.length === 3) {
    // Group by option1
    const option1Groups = new Map<string, VariantFormState[]>();

    flatVariants.forEach(variant => {
      const key = variant.option1 || '';
      if (!option1Groups.has(key)) {
        option1Groups.set(key, []);
      }
      option1Groups.get(key)!.push(variant);
    });

    // Build hierarchy: Option1 (parent) -> Option2/Option3 combinations (children)
    option1Groups.forEach((children, option1Value) => {
      const parentId = generateId(option1Value);

      // Map children and preserve their data
      const childrenWithIds = children.map(child => ({
        ...child,
        // Preserve backend ID if exists, otherwise use deterministic ID
        id: child.id || generateId(child.option1, child.option2, child.option3),
        parentId: parentId,
        children: [],
        isExpanded: true,
        level: 1 // Direct children of Op1 (combined Op2/Op3)
      }));

      // Create parent (Op1 only)
      const parent: VariantFormState = {
        id: parentId,
        option1: option1Value,
        // Parent nodes don't have sellable data, use child's price as reference
        price: children[0]?.price,
        isActive: true,
        position: hierarchicalVariants.length,
        parentId: undefined,
      ...
      hierarchicalVariants.push(parent);
    });

    return hierarchicalVariants;
  }

  // Standard hierarchical build for 1 or 2 options
  if (options.length === 1) {
    // Single option - all variants are top-level (no nesting)
    return flatVariants.map((variant, index) => ({
      ...variant,
      // Preserve backend ID if exists, otherwise use deterministic ID
      id: variant.id || generateId(variant.option1),
      parentId: undefined,
      children: [],
      isExpanded: true,
      level: 0,
  ...
  if (options.length === 2) {
    ...
    // Build hierarchy: Option1 (parent) -> Option2 (children)
    option1Groups.forEach((children, option1Value) => {
      const parentId = generateId(option1Value);
      ...
      const childrenWithIds = children.map(child => ({
        ...child,
        // Preserve backend ID if exists, otherwise use deterministic ID
        id: child.id || generateId(child.option1, child.option2),
        parentId: parentId,
        children: [],
        isExpanded: true,
        level: 1
...
/**
 * Intelligently merge existing variants with new option structure
 * Used when user modifies options in EDIT mode (add/remove option values or options)
 * 
 * This function:
 * 1. Generates new variant structure from updated options
 * 2. Matches existing variants to new variants by option values
 * 3. Preserves data (inventory, images, prices) from matching variants
 * 4. Adds new variants with empty data where no match exists
 * 
 * @param existingVariants - Current hierarchical variants (may have data)
 * @param newOptions - Updated options from user
 * @param basePrice - Base price for new variants
 * @returns Merged hierarchical variants with preserved data where possible
 */
export function mergeVariantsWithNewStructure(
  existingVariants: VariantFormState[],
  newOptions: ProductOption[],
  basePrice?: number
): VariantFormState[] {
  // Generate fresh structure based on new options
  const newStructure = generateVariantCombinations(newOptions, basePrice);

  // Flatten existing variants to make lookup easier
  const flattenExisting = (variants: VariantFormState[]): VariantFormState[] => {
  ...
  const existingFlat = flattenExisting(existingVariants);

  // Helper: Find matching existing variant by option values
  const findMatchingVariant = (
    option1?: string,
    option2?: string,
    option3?: string
  ): VariantFormState | undefined => {
    return existingFlat.find(v => {
      // Match all defined option levels
      const matchesOption1 = !option1 || v.option1 === option1;
      const matchesOption2 = !option2 || v.option2 === option2;
      const matchesOption3 = !option3 || v.option3 === option3;

      // For exact match, check defined options only
      if (option1 && option2 && option3) {
        return v.option1 === option1 && v.option2 === option2 && v.option3 === option3;
      } else if (option1 && option2) {
        return v.option1 === option1 && v.option2 === option2 && !v.option3;
      } else if (option1) {
        return v.option1 === option1 && !v.option2 && !v.option3;
      }
  ...
  // Helper: Copy data from existing variant to new variant
  const mergeData = (
    newVariant: VariantFormState,
    existingVariant?: VariantFormState
  ): VariantFormState => {
    if (!existingVariant) {
      return newVariant; // No match, return as-is
    }

    // Preserve all data fields from existing variant
    return {
      ...newVariant,
      ...
      featuredMediaId: existingVariant.featuredMediaId,
      isActive: existingVariant.isActive,
      id: existingVariant.id, // Keep backend ID if exists
    };
  };

  // Recursively merge new structure with existing data
  const mergeRecursive = (variants: VariantFormState[]): VariantFormState[] => {
    return variants.map(v => {
      // Find matching existing variant
      const match = findMatchingVariant(v.option1, v.option2, v.option3);

      // Merge data from match
      const merged = mergeData(v, match);
  ...
  return mergeRecursive(newStructure);
}
