...
interface AuthStoreState {
  ...
  isLoading: boolean
  isLoggingOut: boolean // Prevents AuthGuard redirect race condition
  error: string | null
    ...
    immer((set, get) => ({
      ...
      setLogoutSuccess: () => {
        ...
        set((state) => {
          ...
          state.isInitialized = false // Reset initialization for fresh login
        })

        // Reset coordinator on logout (allows fresh initialization on next login)
      ...
      initializeSession: async () => {
        // Already initialized AND have user data, skip
        if (get().isInitialized && get().user !== null) {
          TokenRefreshCoordinator.markInitialized()
          return
        }

        // Reset coordinator if Zustand was cleared but coordinator wasn't
        // This handles browser memory pressure scenarios
        if (!get().isInitialized || get().user === null) {
          TokenRefreshCoordinator.reset()
        }

        try {
          // Use coordinator to prevent race conditions with TokenManager
          const result = await TokenRefreshCoordinator.initializeSession()

          if (result.success && result.response?.tokens?.access_token) {
      ...
      refreshTokenSafe: async () => {
        // Check if coordinator allows refresh (blocked during initialization)
        if (!TokenRefreshCoordinator.canTokenManagerRefresh()) {
          // Return a mock response - the initialization will handle the real refresh
          console.log('[authStore] Refresh blocked by coordinator, initialization in progress')
          throw new Error('Refresh blocked during initialization')
        }

        // Use coordinator for de-duplication
        const result = await TokenRefreshCoordinator.refreshToken()

        if (result.success && result.response) {
          get().setRefreshSuccess(result.response)
...
export const authSelectors = {
  ...
  isLoggingOut: (state: AuthStoreState) => state.isLoggingOut,
