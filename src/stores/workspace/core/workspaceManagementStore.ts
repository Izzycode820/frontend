    ...
    immer((set, get) => ({
      ...
      listWorkspaces: async (force = false) => {
        ...
        try {
          ...
          set((state) => {
            const errorMessage = (error as any)?.message || (error as any)?.error || (error instanceof Error ? error.message : 'Failed to fetch workspaces')
      ...
      getWorkspace: async (workspaceId) => {
        ...
        try {
          ...
          set((state) => {
            const errorMessage = (error as any)?.message || (error as any)?.error || (error instanceof Error ? error.message : 'Failed to fetch workspace')
            state.error = errorMessage
            state.isLoading = false
          })
          throw error
