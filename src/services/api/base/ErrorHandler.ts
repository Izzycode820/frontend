...
export class ErrorFactory {
   ...
   * Extracts actual error message from response body if available
   */
  static fromResponse(response: Response, endpoint: string, details?: unknown): APIError {
    ...
    let message = STATUS_CODE_MESSAGES[response.status] || 'Unknown error occurred'
    let code: string | undefined

    if (details && typeof details === 'object') {
      const responseBody = details as Record<string, unknown>

      // Check common error message field names in order of priority
      // Backend typically returns: { "error": "..." } or { "message": "..." } or { "detail": "..." }
      ...
      } else if (typeof responseBody.detail === 'string' && responseBody.detail) {
      ...
      } else if (typeof responseBody.code === 'string') {
        code = responseBody.code
      }
    }

    return new APIError(message, response.status, code, details, endpoint)
  }
