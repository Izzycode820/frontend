...
export class BaseClient {
  ...
  private async executeRequest<T>(
    ...
    try {
      ...
      // CRITICAL FIX: Don't swallow structured backend errors
      // specific error objects (like { error_code: 'CREATION_IN_PROGRESS' }) are thrown by ErrorHandler
      // and must be propagated up to the UI
   ...
   * { success: true, tokens: { access_token: "...", expires_in: 900 }, user: {...} }
   */
  private async attemptTokenRefresh(): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        ...
        // Handle correct response structure: data.tokens.access_token
        const accessToken = data.tokens?.access_token || data.access_token
        const expiresIn = data.tokens?.expires_in || data.expires_in

        if (accessToken) {
          TokenManager.setToken(accessToken, expiresIn)
          console.log('[BaseClient] Token refresh successful')
          return true
        }
      }
    } catch (error) {
      console.warn('[BaseClient] Token refresh failed:', error)
    }
