...
export function useAuth(): UseAuthReturn {
  ...
  const setLoggingOut = useAuthStore((state: any) => state.setLoggingOut)
  ...
  const logout = useCallback(async (): Promise<void> => {
    try {
      setLoading(true)
      // Signal intentional logout to prevent AuthGuard race conditions
      setLoggingOut(true)

      // 1. Clear all Apollo Client Caches (Nuclear Option)
      // We clearStore() instead of resetStore() because we are about to reload anyway
      // and we don't want to trigger refetches.
      await Promise.all([
        adminStoreClient.clearStore(),
        hostinClient.clearStore(),
        themeClient.clearStore(),
        subscriptionClient.clearStore(),
        notificationClient.clearStore(),
      ])

      // 2. Call logout service (handles token cleanup)
      await authService.logout()

      // 3. Update store state (clears sensitive data from memory)
      setLogoutSuccess()

      // 4. Force Hard Reload (The Guarantee)
      // Instead of router.replace(), we force a browser reload.
      // This dumps the JS heap, ensuring no state pollution.
      window.location.href = '/auth/login'

    } catch (error) {
      // Even if logout fails, clear local state and force reload
      setLogoutSuccess()
      window.location.href = '/auth/login'
    } finally {
      // No need to setLoading(false) as we are unloading the page
    }
  }, [setLoading, setLogoutSuccess, setLoggingOut])
